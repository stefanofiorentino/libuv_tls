...
void write_cb(uv_write_t *req, int status) {
    ...
    if (std::string(wr->buf.base).find("endif()")!=std::string::npos)
    {
        if (!uv_is_closing(reinterpret_cast<const uv_handle_t *>(req->handle)))
        {
            uv_close(reinterpret_cast<uv_handle_t *>(req->handle), reinterpret_cast<uv_close_cb>(free));
        }
    }
